...
router.get('/destinations', async (req, res) => {
  try {
    ...
    const visitatoParam = req.query.visitato; // 'true' | 'false' | undefined
    const mine = req.query.mine === 'true';

    // Leggi userId dal token se presente (opzionale, usato per filtro "mine")
...
router.get('/destinations/:id/weather', async (req, res) => {
  try {
    ...
    if (!apiKey) {
      res.status(500).json({ error: 'Servizio meteo non configurato' });
      return;
    }
...
router.post('/destinations/:id/itinerary', async (req, res) => {
  try {
    ...
    if (!apiKey) {
      res.status(500).json({ error: 'Servizio AI non configurato' });
      return;
    }
